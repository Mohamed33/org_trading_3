# -*- coding: utf-8 -*-
#################################################################################
# Author      : Acespritech Solutions Pvt. Ltd. (<www.acespritech.com>)
# Copyright(c): 2012-Present Acespritech Solutions Pvt. Ltd.
# All Rights Reserved.
#
# This program is copyright property of the author mentioned above.
# You can`t redistribute it and/or modify it.
#
#################################################################################
from odoo import _, api, exceptions, fields, models
from odoo.exceptions import UserError
from odoo.tests.common import Form


class Settlement(models.Model):
    _name = "purchase.commission.settlement"
    _description = "Settlement"

    def _default_currency(self):
        return self.env.user.company_id.currency_id.id

    name = fields.Char("Name")
    total = fields.Float(compute="_compute_total", readonly=True, store=True)
    date_from = fields.Date(string="From")
    date_to = fields.Date(string="To")
    purchase_agent_id = fields.Many2one(
        comodel_name="res.partner", domain="[('purchase_agent', '=', True)]"
    )
    purchase_agent_type = fields.Selection(related="purchase_agent_id.purchase_agent_type")
    line_ids = fields.One2many(
        comodel_name="purchase.commission.settlement.line",
        inverse_name="purchase_settlement_id",
        string="Settlement lines",
        readonly=True,
    )
    state = fields.Selection(
        selection=[
            ("settled", "Settled"),
            ("invoiced", "Invoiced"),
            ("cancel", "Canceled"),
            ("except_invoice", "Invoice exception"),
        ],
        string="State",
        readonly=True,
        default="settled",
    )
    purchase_invoice_ids = fields.One2many(
        comodel_name="account.move",
        inverse_name="purchase_settlement_id",
        string="Generated invoice",
        readonly=True,
    )
    # TODO: To be removed
    purchase_invoice_id = fields.Many2one(
        store=True, comodel_name="account.move", compute="_compute_purchase_invoice_id",
    )
    currency_id = fields.Many2one(
        comodel_name="res.currency", readonly=True, default=_default_currency
    )
    company_id = fields.Many2one(
        comodel_name="res.company",
        default=lambda self: self.env.user.company_id,
        required=True,
    )

    @api.depends("line_ids", "line_ids.settled_amount")
    def _compute_total(self):
        for record in self:
            record.total = sum(record.mapped("line_ids.settled_amount"))

    @api.depends("purchase_invoice_ids")
    def _compute_purchase_invoice_id(self):
        for record in self:
            record.purchase_invoice_id = record.purchase_invoice_ids[:1]

    def action_cancel(self):
        if any(x.state != "settled" for x in self):
            raise exceptions.Warning(_("Cannot cancel an invoiced settlement."))
        self.write({"state": "cancel"})

    def unlink(self):
        """Allow to delete only cancelled settlements"""
        if any(x.state == "invoiced" for x in self):
            raise exceptions.Warning(_("You can't delete invoiced settlements."))
        return super().unlink()

    def action_invoice(self):
        return {
            "type": "ir.actions.act_window",
            "name": _("Make invoice"),
            "res_model": "purchase.commission.make.invoice",
            "view_type": "form",
            "target": "new",
            "view_mode": "form",
            "context": {"purchase_settlement_ids": self.ids},
        }

    def _prepare_invoice(self, journal, product, date=False):
        self.ensure_one()
        move_type = "in_invoice" if self.total >= 0 else "in_refund"
        journal = self.env['account.move'].with_context(default_move_type=move_type)._get_default_journal()
        # move_form = Form(self.env["account.move"].with_context(default_type=move_type))
        # if date:
        #     move_form.invoice_date = date
        # move_form.partner_id = self.purchase_agent_id
        # print("\n\n\n\n\n========move_form.partner_id=========", move_form.partner_id)
        # move_form.journal_id = journal
        # print("\n\n\n\n\n========move_form.journal_id=========", move_form.journal_id)
        # with move_form.invoice_line_ids.new() as line_form:
        #     line_form.product_id = product
        #     line_form.quantity = 1
        #     line_form.price_unit = abs(self.total)
        #     # Put period string
        #     partner = self.purchase_agent_id
        #     lang = self.env["res.lang"].search(
        #         [("code", "=", partner.lang or self.env.context.get("lang", "en_US"))]
        #     )
        #     date_from = fields.Date.from_string(self.date_from)
        #     date_to = fields.Date.from_string(self.date_to)
        #     print("\n\n\n\n\n========date_from=========", date_from,date_to)
        #     line_form.name = "\n" + _("Period: from %s to %s") % (
        #         date_from.strftime(lang.date_format),
        #         date_to.strftime(lang.date_format),
        #     )
        #     print("\n\n\n\n\n========line_form.name=========", line_form.name)
        # print("\n\n\n\n\n=======move_from====",move_form)
        # vals = move_form._values_to_save(all_fields=True)
        # print("\n\n\n\n\\n==========vals=========",vals)
        partner = self.purchase_agent_id
        lang = self.env["res.lang"].search(
                    [("code", "=", partner.lang or self.env.context.get("lang", "en_US"))]
                )
        date_from = fields.Date.from_string(self.date_from)
        date_to = fields.Date.from_string(self.date_to)
        name = "\n" + _("Period: from %s to %s") % (
            date_from.strftime(lang.date_format),
            date_to.strftime(lang.date_format),
        )
        invoice_vals = {
            'move_type': move_type,
            'partner_id': self.purchase_agent_id,
            'invoice_date': date,
            'journal_id':journal.id,
            'invoice_line_ids': [(0, 0, {
                'product_id':product,
                'name': name,
                'quantity': 1,
                'price_unit':  abs(self.total),
                'tax_ids': [(6, 0, [])],
            })]
        }
        invoice_vals["purchase_settlement_id"] = self.id
        return invoice_vals

    def make_invoices(self, journal, product, date=False):
        invoice_vals_list = []
        for settlement in self:
            invoice_vals = settlement._prepare_invoice(journal, product, date)
            invoice_vals_list.append(invoice_vals)
        invoices = self.env["account.move"].create(invoice_vals_list)
        self.write({"state": "invoiced"})
        return invoices


class SettlementLine(models.Model):
    _name = "purchase.commission.settlement.line"
    _description = "Line of a commission settlement"

    purchase_settlement_id = fields.Many2one(
        "purchase.commission.settlement", readonly=True, ondelete="cascade", required=True
    )
    purchase_agent_line = fields.Many2many(
        comodel_name="purchase.account.invoice.line.agent",
        relation="purchase_settlement_agent_line_rel",
        column1="purchase_settlement_id",
        column2="purchase_agent_line_id",
        required=True,
    )
    date = fields.Date(related="purchase_agent_line.invoice_date", store=True)
    invoice_line_id = fields.Many2one(
        comodel_name="account.move.line",
        store=True,
        related="purchase_agent_line.purchase_object_id",
        string="Source invoice line",
    )
    purchase_agent_id = fields.Many2one(
        comodel_name="res.partner",
        readonly=True,
        related="purchase_agent_line.purchase_agent_id",
        store=True,
    )
    settled_amount = fields.Monetary(
        related="purchase_agent_line.amount", readonly=True, store=True
    )
    currency_id = fields.Many2one(
        related="purchase_agent_line.currency_id", store=True, readonly=True,
    )
    purchase_commission_id = fields.Many2one(
        comodel_name="purchase.commission", related="purchase_agent_line.purchase_commission_id"
    )
    company_id = fields.Many2one(
        comodel_name="res.company", related="purchase_settlement_id.company_id",
    )

    @api.constrains("purchase_settlement_id", "purchase_agent_line")
    def _check_company(self):
        for record in self:
            if record.purchase_agent_line.company_id != record.company_id:
                raise UserError(_("Company must be the same"))

# vim:expandtab:smartindent:tabstop=4:softtabstop=4:shiftwidth=4: